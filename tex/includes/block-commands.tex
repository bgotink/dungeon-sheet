...
%  \node[inner sep=0pt,outer sep=0pt,text width=\columnwidth-2*\dndpaddingsize,draw=blue!80,thick](t){#1};
  ...
  \path
    (0,0)
    -- (\columnwidth-2*\dndpaddingsize,0)
    node[midway, below, inner sep=0pt, outer sep=0pt, minimum width=\columnwidth-2*\dndpaddingsize, draw=blue!80, thick] (t) {#1}
  ;
...
\newcommand{\dnd@LinesVerticalThin}{%
  \draw[line width=\dndmedline, shorten <= \dndveroffset, shorten >= \dndveroffset]
    ($ (o) + (\dndverlinesep,0) $)
    -- ($ (c) + (\dndverlinesep,0) $);
  \draw[line width=\dndmedline, shorten <= \dndveroffset, shorten >= \dndveroffset]
    ($ (b) - (\dndverlinesep,0) $)
